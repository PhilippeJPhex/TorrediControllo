...
/* ============================================================
   ...
   Versione Vercel: login, proxy LLM server-side (/api/claude),
   persistenza su Vercel KV (/api/data), KPI automatici da N8N
   (/api/ingest). Estetica cream/ink Calamai, accento #C93741.
   ============================================================ */
...
export default function App() {
  ...
  const saveAll = useCallback(async (a, k, p, i) => {
    try {
      await api("/api/data", {
        method: "POST",
        body: JSON.stringify({ activities: a, kpis: k, plan: p, ispo: i }),
      });
    } catch (e) {
  ...
  const addActivity = async () => {
    ...
    try {
      const txt = await callClaude(
        `${BRAND_CONTEXT}\n\nSei il PR director del brand. Valuta questa attività di comunicazione verso lo stakeholder "${STAKEHOLDER_DEFS.find((s) => s.id === addFor)?.label}":\nTitolo: ${form.title}\nNote: ${form.notes || "—"}\n\nRispondi SOLO con JSON puro, senza backtick né testo extra:\n{"importance": <1-5, quanto pesa per costruire relazione e notorietà>, "rationale": "<max 15 parole in italiano>"}`,
        false,
        400
      );
  ...
  const generatePlan = async () => {
    ...
    try {
      ...
      setPlan(p);
      saveAll(activities, kpis, p, ispo);
    } catch (e) {
  ...
  const generateIspo = async () => {
    ...
    try {
      ...
      /* FASE 1 — ricerca web in prosa */
      const research = await callClaude(
        `Cerca sul web le attività di comunicazione RECENTI dei microbrand orologieri comparabili a un artigiano italiano di orologi (Unimatic, Furlan Marri, Venezianico, Baltic e simili): campagne social, uscite stampa, collaborazioni, lanci di prodotto. Riassumi in italiano cosa hai trovato, brand per brand, in modo conciso.`,
        true,
        2500
      );
      if (!research || research.trim().length < 50) throw new Error("ricerca vuota");

      /* FASE 2 — strutturazione JSON */
      const txt = await callClaude(
        `${BRAND_CONTEXT}\n\nAttività di comunicazione già svolte da Calamai:\n${done}\n\nRisultati di una ricerca di mercato sui microbrand comparabili:\n${research}\n\nSulla base di questi risultati: (1) sintetizza le osservazioni sui competitor, (2) proponi 4-6 idee di comunicazione per Calamai ispirate a ciò che funziona, adattate al suo posizionamento (acciaio da turbine militari, artigianato toscano, conto visione), evitando di ripetere attività già svolte. Rispondi SOLO con JSON puro, senza backtick né testo extra:\n{"observations":[{"who":"<brand>","what":"<cosa hanno fatto, max 20 parole>"}],"ideas":[{"title":"<idea, max 10 parole>","detail":"<come applicarla a Calamai, max 30 parole>","effort":"<basso|medio|alto>"}]}`,
        false,
        1800
      );
      const j = extractJSON(txt);
      const i = { generatedAt: new Date().toISOString(), liveSearch: true, ...j };
      setIspo(i);
      saveAll(activities, kpis, plan, i);
    } catch (e) {
  ...
  const addKpi = () => {
    ...
    setKpis(next);
    saveAll(activities, next, plan, ispo);
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: T.grey, margin: "4px 0 0" }}>Ricerca web live su cosa fanno i microbrand comparabili, con idee adattate a Calamai.</p>
              </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
                  {ispo.observations.map((o, i) => (
                    <div key={i} style={{ ...S.card, padding: 12 }}>
                      <div style={{ ...S.mono, fontSize: 10, color: T.red }}>{o.who}</div>
                      <div style={{ fontSize: 13, marginTop: 4 }}>{o.what}</div>
                    </div>
                  ))}
                </div>
                <div style={{ ...S.mono, fontSize: 11, fontWeight: 700, margin: "20px 0 8px" }}>Idee per Calamai</div>
            ...
            {!ispo && !ispoLoading && <p style={{ fontSize: 13, color: T.grey, marginTop: 20 }}>Avvia la prima scansione per vedere cosa si muove tra i microbrand.</p>}
          </div>
        )}
